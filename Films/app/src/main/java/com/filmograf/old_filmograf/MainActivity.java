...
public class MainActivity extends AppCompatActivity {
    ...
    public void saveUserInBd(FirebaseUser user) {
        String email = user.getEmail();
        new Thread() {
            @Override
            public void run() {
                if (!users_db.user_manager().mailExists(email)) {
                    int user_id = users_db.user_manager().getNumberOfRows();
                    users_db.user_manager().insert(new Users(user_id, email));
                }
            }
        }.start();
    }
